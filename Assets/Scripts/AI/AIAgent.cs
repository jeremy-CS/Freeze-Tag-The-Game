...
public class AIAgent : MonoBehaviour
{
    ...
    //Find closest node
    public Node FindClosestNode(Transform agent)
    {
        float distance = (MapGen.Map.graphNodes[0].Position - agent.position).magnitude;
        Node startNode = MapGen.Map.graphNodes[0];

        foreach (Node node in MapGen.Map.graphNodes)
        {
